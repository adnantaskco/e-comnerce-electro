      ...
      <div className="container mx-auto px-2 sm:px-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {AllProducts.map((item) => (
            <div
              key={item.id}
              className="group relative rounded-xl overflow-hidden bg-white border hover:shadow-xl transition-all duration-300"
            >
              {/* Wishlist */}
              <button
              ...
              <Link href={`/products/${item.id}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className="text-lg font-semibold mt-1 whitespace-nowrap overflow-hidden text-ellipsis">
                    {item.name}
                  </h3>
                </div>
